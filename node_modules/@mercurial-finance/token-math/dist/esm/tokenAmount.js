 ...
 */
exports.DEFAULT_LOCALE_DECIMAL_SEPARATOR = (0, exports.getDecimalSeparator)() ?? ".";
/**
 * The group separator of the default locale.
 */
exports.DEFAULT_LOCALE_GROUP_SEPARATOR = (0, exports.getGroupSeparator)() ?? ",";
/**
...
const parseAmountFromString = (token, uiAmount, decimalSeparator = exports.DEFAULT_DECIMAL_SEPARATOR, groupSeparator = exports.DEFAULT_GROUP_SEPARATOR) => {
    ...
    const fraction = fractionRaw
        ? BigInt(fractionRaw.slice(0, token.decimals) +
            Array(token.decimals).fill("0").slice(fractionRaw.length).join(""))
        : constants_js_1.ZERO;
    const combined = whole * (0, utils_js_1.makeDecimalMultiplier)(token.decimals) + fraction;
    return combined;
};
...
function validateU64(value) {
    if (!(value >= constants_js_1.ZERO)) {
        throw new TokenAmountUnderflow(value);
    }
    if (!(value <= constants_js_1.MAX_U64)) {
...
function validateU256(value) {
    ...
    if (!(value <= constants_js_1.MAX_U256)) {
